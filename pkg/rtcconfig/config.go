// Copyright 2023 LiveKit, Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package rtcconfig

import (
	"encoding/json"
	"fmt"
	"strconv"
	"strings"
	"time"

	"github.com/im-pingo/mediatransportutil/pkg/logger"
	"gopkg.in/yaml.v3"
)

const (
	// number of packets to buffer up
	readBufferSize = 50

	minUDPBufferSize       = 5_000_000
	writeBufferSizeInBytes = 4 * 1024 * 1024
	defaultUDPBufferSize   = 16_777_216
)

var DefaultStunServers = []string{
	"stun.miwifi.com:3478",        // Xiaomi STUN server
	"stun.qq.com:3478",            // Tencent STUN server
	"stun.chat.bilibili.com:3478", // Bilibili STUN server
	"stun.ideasip.com:3478",       // IdeaSIP - international but good connectivity in Asia
	"stun.voipgate.com:3478",      // VoIPGate - good Asian coverage
	"stun.l.google.com:19302",     // Google (keep as backup)
	"stun1.l.google.com:19302",    // Google (keep as backup)
}

type RTCConfig struct {
	UDPPort                 PortRange        `yaml:"udp_port,omitempty" json:"udp_port,omitempty"`
	TCPPort                 uint32           `yaml:"tcp_port,omitempty" json:"tcp_port,omitempty"`
	ICEPortRangeStart       uint32           `yaml:"port_range_start,omitempty" json:"port_range_start,omitempty"`
	ICEPortRangeEnd         uint32           `yaml:"port_range_end,omitempty" json:"port_range_end,omitempty"`
	NodeIP                  string           `yaml:"node_ip,omitempty" json:"node_ip,omitempty"`
	NodeIPAutoGenerated     bool             `yaml:"-" json:"-"`
	STUNServers             []string         `yaml:"stun_servers,omitempty" json:"stun_servers,omitempty"`
	UseExternalIP           bool             `yaml:"use_external_ip" json:"use_external_ip"`
	UseICELite              bool             `yaml:"use_ice_lite,omitempty" json:"use_ice_lite,omitempty"`
	Interfaces              InterfacesConfig `yaml:"interfaces,omitempty" json:"interfaces,omitempty"`
	IPs                     IPsConfig        `yaml:"ips,omitempty" json:"ips,omitempty"`
	EnableLoopbackCandidate bool             `yaml:"enable_loopback_candidate" json:"enable_loopback_candidate"`
	UseMDNS                 bool             `yaml:"use_mdns,omitempty" json:"use_mdns,omitempty"`
	ExternalIPOnly          bool             `yaml:"external_ip_only,omitempty" json:"external_ip_only,omitempty"`
	BatchIO                 BatchIOConfig    `yaml:"batch_io,omitempty" json:"batch_io,omitempty"`
	// for testing, disable UDP
	ForceTCP bool `yaml:"force_tcp,omitempty" json:"force_tcp,omitempty"`
}

type InterfacesConfig struct {
	Includes []string `yaml:"includes,omitempty" json:"includes,omitempty"`
	Excludes []string `yaml:"excludes,omitempty" json:"excludes,omitempty"`
}

type IPsConfig struct {
	Includes []string `yaml:"includes,omitempty" json:"includes,omitempty"`
	Excludes []string `yaml:"excludes,omitempty" json:"excludes,omitempty"`
}

type BatchIOConfig struct {
	BatchSize        int           `yaml:"batch_size,omitempty" json:"batch_size,omitempty"`
	MaxFlushInterval time.Duration `yaml:"max_flush_interval,omitempty" json:"max_flush_interval,omitempty"`
}

type PortRange struct {
	Start int `yaml:"start,omitempty" json:"start,omitempty"`
	End   int `yaml:"end,omitempty" json:"end,omitempty"`
}

func (conf *RTCConfig) Validate(development bool) error {
	// set defaults for ports if none are set
	if !conf.UDPPort.Valid() && conf.ICEPortRangeStart == 0 {
		// to make it easier to run in dev mode/docker, default to single port
		if development {
			conf.UDPPort = PortRange{Start: 7882}
		} else {
			conf.ICEPortRangeStart = 50000
			conf.ICEPortRangeEnd = 60000
		}
	}

	var err error
	if conf.NodeIP == "" || conf.UseExternalIP {
		conf.NodeIP, err = conf.determineIP()
		if err != nil {
			logger.Warn("could not determine node ip", err)
			return err
		}
		conf.NodeIPAutoGenerated = true
	}

	return nil
}

func (r PortRange) MarshalYAML() (interface{}, error) {
	if r.End == 0 {
		return r.Start, nil
	}

	if r.End <= r.Start {
		return nil, fmt.Errorf("end port %d must be greater than start port %d", r.End, r.Start)
	}
	return fmt.Sprintf("%d-%d", r.Start, r.End), nil
}

func (r *PortRange) UnmarshalYAML(value *yaml.Node) error {
	return r.UnmarshalString(value.Value)
}

func (r *PortRange) UnmarshalString(str string) error {
	if str == "" {
		return nil
	}
	if strings.Contains(str, "-") {
		parts := strings.Split(str, "-")
		if len(parts) != 2 {
			return fmt.Errorf("invalid port range %s, should be <start>-<end>", str)
		}
		start, err := strconv.Atoi(strings.Trim(parts[0], " "))
		if err != nil {
			return fmt.Errorf("invalid start port %s: %v", parts[0], err)
		}
		end, err := strconv.Atoi(strings.Trim(parts[1], " "))
		if err != nil {
			return fmt.Errorf("invalid end port %s: %v", parts[1], err)
		}
		if end <= start {
			return fmt.Errorf("end port %d must be greater than start port %d", end, start)
		}

		r.Start = start
		r.End = end
		return nil
	}

	port, err := strconv.Atoi(str)
	if err != nil {
		return fmt.Errorf("invalid port %s: %v", str, err)
	}
	r.Start = port
	return nil
}

func (r PortRange) MarshalJSON() ([]byte, error) {
	if r.End == 0 {
		return json.Marshal(r.Start)
	}

	if r.End <= r.Start {
		return nil, fmt.Errorf("end port %d must be greater than start port %d", r.End, r.Start)
	}
	return json.Marshal(fmt.Sprintf("%d-%d", r.Start, r.End))
}

func (r *PortRange) UnmarshalJSON(data []byte) error {
	var raw interface{}
	if err := json.Unmarshal(data, &raw); err != nil {
		return err
	}

	switch v := raw.(type) {
	case float64:
		r.Start = int(v)
		return nil
	case string:
		return r.UnmarshalString(v)
	default:
		return fmt.Errorf("invalid port range format: %v", raw)
	}
}

func (r *PortRange) ToSlice() []int {
	if r.End == 0 || r.End <= r.Start {
		return []int{r.Start}
	}

	ports := make([]int, r.End-r.Start+1)
	for i := r.Start; i <= r.End; i++ {
		ports[i-r.Start] = i
	}
	return ports
}

func (r *PortRange) Valid() bool {
	return r.Start != 0
}
